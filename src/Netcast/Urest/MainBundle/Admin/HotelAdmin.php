<?php
    ...
    class HotelAdmin extends Admin
    {
        ...
        public function prePersist($item)
        {
            ...
            foreach($item->getHotelContent() as $hotelContent) {
                if(!$hotelContent->getIsDeleted())
                    $hotelContent->setParent($item);
                else
                    $item->removeCityContent($hotelContent);
            }
        }
        ...
        protected function configureFormFields(FormMapper $formMapper)
        {
            ...
            $formMapper
                ...
                ->add('hotelContent', 'urest_i18n_collection', [
                    'label' => 'form.label.content',
                    'type' => 'netcast_urest_title_content_form',
                    'options' => [
                        'label' => false,
                        'required' => false,
                        'data_class' => 'Netcast\Urest\MainBundle\Entity\HotelContent',
                    ],
                    'by_reference' => false,
                    'allow_add' => true,
                    'allow_delete' => false,
